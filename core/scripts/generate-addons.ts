...
function generateProvidersFile(
    ...
    const withProviders = addonsInfo.filter(a => a.manifest.provider?.component)
    ...
    return [
        ...
        '}',
        '',
        'export const addonProviders: Record<string, ComponentType<AddonProviderProps> | null> = {',
        ...entries,
        '}',
        '',
    ].join('\n')
...
async function main() {
    ...
    const providersFile = path.join(GENERATED_DIR, 'addon-providers.ts')
    fs.writeFileSync(providersFile, generateProvidersFile(addonsInfo))
