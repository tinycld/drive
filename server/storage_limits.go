...
func getUserStorageUsed(app core.App, userID string) (int64, error) {
	var result struct {
	...
	err := app.DB().NewQuery(`
		SELECT COALESCE(
			(SELECT SUM(size) FROM drive_items WHERE created_by = {:userID}), 0
		) + COALESCE(
			(SELECT SUM(v.size) FROM drive_item_versions v
			 JOIN drive_items i ON v.item = i.id
			 WHERE v.created_by = {:userID}), 0
		) AS total
	`).Bind(map[string]any{"userID": userID}).One(&result)
	if err != nil {
		return 0, err
	}
...
func getDeploymentStorageUsed(app core.App) (int64, int64, error) {
	...
	err := app.DB().NewQuery(`
		SELECT COALESCE(
			(SELECT SUM(size) FROM drive_items), 0
		) + COALESCE(
			(SELECT SUM(v.size) FROM drive_item_versions v
			 JOIN drive_items i ON v.item = i.id), 0
		) AS total
...
func getStorageLimitBytes(app core.App) int64 {
	var result struct {
		Value int64 `db:"val"`
	}
	err := app.DB().NewQuery(`
		...
		FROM settings
		WHERE app = 'core' AND key = 'storage_limit_bytes'
		LIMIT 1
	`).Bind(map[string]any{}).One(&result)
	if err != nil {
		return 0
	}
	return result.Value
...
// checkUserStorageQuota returns an error if the user would exceed their limit by adding additionalBytes.
// Returns nil if unlimited or within limit.
func checkUserStorageQuota(app core.App, userID string, additionalBytes int64) error {
	limit := getStorageLimitBytes(app)
	if limit <= 0 {
		return nil
	}

	used, err := getUserStorageUsed(app, userID)
	if err != nil {
...
func checkUserStorageQuotaWebDAV(app core.App, userID string, additionalBytes int64) error {
	if err := checkUserStorageQuota(app, userID, additionalBytes); err != nil {
