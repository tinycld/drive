...
migrate(
    app => {
        ...
        const hasShareRule = 'item.drive_shares_via_item.user_org ?= @request.auth.id'
        const isOwnerOrEditor =
            'item.drive_shares_via_item.user_org ?= @request.auth.id && item.drive_shares_via_item.role ?!= "viewer"'
        const isOwner =
